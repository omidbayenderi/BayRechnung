...
const TABLE_SCHEMAS = {
    ...
    sales: ['id', 'user_id', 'customer_name', 'total', 'payment_method', 'items', 'status', 'reported_to_accounting', 'created_at'],
    ...
    appointments: [
        ...
        'service_id', 'staff_id', 'date', 'start_time', 'end_time', 'status', 'notes',
        'reminded', 'processed_by_ai', 'created_at'
    ],
    company_settings: [
        ...
        'default_currency', 'default_tax_rate', 'tax_exempt', 'paypal_me', 'stripe_link',
...
class SyncService {
    ...
    enqueue(table, action, data, id = null) {
        ...
        if (!userId && (action === 'insert' || action === 'update')) {
            console.warn(`[Sync] Attempted to enqueue ${table} without a valid user_id. Action: ${action}`);
        }
    ...
    async processQueue(force = false) {
        ...
        if (!isOnline) return;
        if (!this.isConfigured && !isSupabaseConfigured()) return;
        ...
        try {
            ...
            console.error("[Sync] Queue processing exception:", err);
        } finally {
            this.isProcessing = false;
            this.notifyListeners();
        }
    ...
    async syncItem(item) {
        try {
            ...
            if (action === 'insert' || action === 'update') {
                ...
                if (action === 'insert' && !finalData.user_id && !finalData.sender_id) {
                    return { success: false, error: { message: 'Missing user_id for insert', code: 'APP_MISSING_ID' } };
                }

                if (finalTargetId && finalData && !finalData.id && conflictTarget === 'id') {
                    finalData.id = finalTargetId;
                }

                if (action === 'insert') {
                    query = supabase.from(table).upsert(finalData, { onConflict: conflictTarget });
                } else {
                    if (!finalTargetId) return { success: false, error: { message: `Missing targetId for ${action} on ${table}`, code: 'APP_MISSING_TARGET' } };
                    query = supabase.from(table).update(finalData).eq(conflictTarget, finalTargetId);
                }
            } else if (action === 'delete') {
                query = supabase.from(table).delete().eq('id', targetId);
            }

            if (!query) return { success: false, error: { message: 'Invalid action/query' } };

            const { error } = await query;
            if (error) {
                console.error(`🛑 [Sync] DB Error on ${table} (${action}):`, error.message, error.code);
                const errorMsg = error.message || "Unknown error";
                if (!errorMsg.includes('FetchError') && !errorMsg.includes('Network Error') && !errorMsg.includes('Failed to fetch')) {
                    if (!this.errors.find(e => e.message === errorMsg)) {
                        this.errors.push({ id: Date.now(), table, action, message: errorMsg, code: error.code, timestamp: new Date().toISOString() });
                        this.notifyListeners();
                    }
                }
            ...
            console.error("[Sync] syncItem exception:", e);
            this.lastSuccess = 0;
            return { success: false, error: { message: e.message, code: 'EXCEPTION' } };
        }
    }
